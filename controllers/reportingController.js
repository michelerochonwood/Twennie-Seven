...
const memberEngagementCSV = async (req, res) => {
  try {
    // Reuse the same data-building logic from getMemberEngagementReport
    // If that logic is inline, you can call it via a shared function;
    // below we inline a small fetch mirroring your HTML handler:
    const leader = await Leader.findById(req.user.id).select('groupName').lean();
    ...
    const completions = await PromptSetCompletion
      .find({ memberId: { $in: memberIds } })
      .populate('promptSetId', 'promptset_title')
      .lean();

    const notes = await require('../models/notes/notes')
      .find({ memberID: { $in: memberIds } })
      .lean();
    ...
    const [
      ...
      Article.find({ 'author.id': { $in: memberIds } }).select('article_title author.id').lean(),
      Video.find({ 'author.id': { $in: memberIds } }).select('video_title author.id').lean(),
      Interview.find({ 'author.id': { $in: memberIds } }).select('interview_title author.id').lean(),
      Exercise.find({ 'author.id': { $in: memberIds } }).select('exercise_title author.id').lean(),
      Template.find({ 'author.id': { $in: memberIds } }).select('template_title author.id').lean(),
      PromptSet.find({ 'author.id': { $in: memberIds } }).select('promptset_title author.id').lean()
    ]);

    // Flatten to rows (one row per "member x metric"):
    const rows = [];

    // Prompt sets completed
    for (const c of completions) {
      rows.push({
        Member: nameById.get(String(c.memberId)) || 'Unknown Member',
        Metric: 'Prompt Set Completed',
        Item: c.promptSetId?.promptset_title || 'Unknown Prompt Set',
        Extra: '',
        Date: c.completedAt || c.createdAt
      });
      if (c.earnedBadge?.name) {
        rows.push({
          Member: nameById.get(String(c.memberId)) || 'Unknown Member',
          Metric: 'Badge Earned',
          Item: c.earnedBadge.name,
          Extra: c.earnedBadge.image || '',
          Date: c.completedAt || c.createdAt
        });
      }
    }
    ...
    for (const n of notes) {
      const det = await resolveUnitDetails(n.unitID);
      rows.push({
        Member: nameById.get(String(n.memberID)) || 'Unknown Member',
        Metric: 'Unit Completed',
        Item: `${det.unitTitle} (${det.unitType})`,
        Extra: (det.main_topic || ''),
        Date: n.createdAt || n.updatedAt
      });
    }

    // Units contributed (authored)
    function addContrib(list, typeKey) {
      for (const u of list) {
        rows.push({
          Member: nameById.get(String(u.author?.id)) || 'Unknown Member',
          Metric: 'Unit Contributed',
          Item: (u[ typeKey ] || 'Untitled') + ' (' + typeKey.replace('_title','').replace('promptset','Prompt Set').replace('video','Video').replace('article','Article').replace('interview','Interview').replace('exercise','Exercise').replace('template','Template') + ')',
          Extra: '',
          Date: ''
        });
      }
    }
    addContrib(art, 'article_title');
    addContrib(vid, 'video_title');
    addContrib(intv, 'interview_title');
    addContrib(ex, 'exercise_title');
    addContrib(tpl, 'template_title');
    addContrib(psets, 'promptset_title');
    ...
    res.setHeader('Content-Disposition', `attachment; filename="member_engagement_${Date.now()}.csv"`);
    return res.send(csv);
  } catch (e) {
    console.error('memberEngagementCSV error:', e);
    return res.status(500).send('CSV export failed');
  }
};

// GET /reports/teamengagement.csv
const teamEngagementCSV = async (req, res) => {
  try {
    const leader = await Leader.findById(req.user.id).select('groupName').lean();
    const members = await GroupMember.find({ groupId: req.user.id }).select('_id name').lean();
    const memberIds = members.map(m => m._id);
    ...
    // Prompt sets completed (include member name so it’s meaningful)
    for (const p of promptCompletions) {
      rows.push({
        Section: 'Prompt Sets Completed',
        Item: p.promptSetId?.promptset_title || 'Unknown Prompt Set',
        Member: nameById.get(String(p.memberId)) || 'Unknown Member',
        Date: p.completedAt || p.createdAt
      });
      if (p.earnedBadge?.name) {
        rows.push({
          Section: 'Badges Earned',
          Item: p.earnedBadge.name,
          Member: nameById.get(String(p.memberId)) || 'Unknown Member',
          Date: p.completedAt || p.createdAt
        });
      }
    ...
    // Units completed from Notes
    for (const n of notes) {
      const det = await resolveUnitDetails(n.unitID);
      rows.push({
        Section: 'Units Completed',
        Item: `${det.unitTitle} (${det.unitType})`,
        Member: nameById.get(String(n.memberID)) || 'Unknown Member',
        Date: n.createdAt || n.updatedAt
      });
    ...
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="team_engagement_${Date.now()}.csv"`);
    return res.send(csv);
  } catch (e) {
    console.error('teamEngagementCSV error:', e);
    return res.status(500).send('CSV export failed');
  }
};


// GET /reports/promptsetscompleted.csv
const promptSetsCompletedCSV = async (req, res) => {
  try {
    const members = await GroupMember.find({ groupId: req.user.id }).select('_id name').lean();
    const memberIds = members.map(m => m._id);
    const nameById = new Map(members.map(m => [String(m._id), m.name]));
    ...
    const rows = completions.map(c => ({
      PromptSet: c.promptSetId?.promptset_title || 'Unknown Prompt Set',
      Member: nameById.get(String(c.memberId)) || 'Unknown Member',
      Badge: c.earnedBadge?.name || '',
      BadgeImage: c.earnedBadge?.image || '',
      CompletedAt: c.completedAt || c.createdAt
    }));

    const csv = toCSV(rows, [
      { key: 'PromptSet',  header: 'Prompt Set' },
      { key: 'Member',     header: 'Member' },
      { key: 'Badge',      header: 'Badge' },
      { key: 'BadgeImage', header: 'Badge Image' },
      { key: 'CompletedAt',header: 'Completion Date' }
    ]);

    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="prompt_sets_completed_${Date.now()}.csv"`);
    return res.send(csv);
  } catch (e) {
    console.error('promptSetsCompletedCSV error:', e);
    return res.status(500).send('CSV export failed');
  }
};

// GET /reports/unitscompleted.csv
const unitsCompletedCSV = async (req, res) => {
  try {
    const members = await GroupMember.find({ groupId: req.user.id }).select('_id name').lean();
    const memberIds = members.map(m => m._id);
    const nameById = new Map(members.map(m => [String(m._id), m.name]));
    const notes = await require('../models/notes/notes').find({ memberID: { $in: memberIds } }).lean();

    const rows = [];
    for (const n of notes) {
      const det = await resolveUnitDetails(n.unitID);
      rows.push({
        UnitTitle: det.unitTitle,
        UnitType: det.unitType,
        MainTopic: det.main_topic || '',
        SecondaryTopics: (det.secondary_topics || []).join('; '),
        Member: nameById.get(String(n.memberID)) || 'Unknown Member',
        Note: n.note_content || '',
        DateSubmitted: n.createdAt || n.updatedAt
      });
    }

    const csv = toCSV(rows, [
      { key: 'UnitTitle',       header: 'Unit Title' },
      { key: 'UnitType',        header: 'Unit Type' },
      { key: 'MainTopic',       header: 'Main Topic' },
      { key: 'SecondaryTopics', header: 'Secondary Topics' },
      { key: 'Member',          header: 'Member' },
    ...
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="units_completed_${Date.now()}.csv"`);
    return res.send(csv);
  } catch (e) {
    console.error('unitsCompletedCSV error:', e);
    return res.status(500).send('CSV export failed');
  }
};
